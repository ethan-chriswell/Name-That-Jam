(function(){
  ...
  $("year-answer-form").onsubmit=e=>{e.preventDefault();socket.emit("answer",{year:+$("year-answer").value,artist:$("artist-bonus").value.trim()})};
  ...
  function render(s){state=s;
    ...
    const yearMode=s.settings.mode==="year"&&!s.isHost;if(yearMode&&yearRound!==s.round){yearRound=s.round;$("year-answer").value="";$("artist-bonus").value=""}$("year-answer-form").hidden=!yearMode;$("year-answer").disabled=s.answered;$("artist-bonus").disabled=s.answered;$("question-prompt").textContent=yearMode?"What year was it released?":"What song is this?";if(yearMode&&!s.answered&&s.phase==="question")$("year-answer").focus();
    ...
    if(s.answer){$("answer-title").textContent=s.answer.title;$("answer-artist").textContent=s.answer.artist;$("answer-year").textContent=s.settings.mode==="year"?`Released ${s.answer.releaseYear}`:""}$("round-results").innerHTML=s.phase==="reveal"?s.players.map(p=>{const result=p.roundCorrect?`+${p.roundPoints}`:p.roundArtistBonus?`Artist bonus +${p.roundPoints}`:'Wrong';return `<li><span class="result-icon ${p.roundCorrect?'right':'wrong'}">${p.roundCorrect?'✓':'✕'}</span><span class="result-name">${escapeHtml(p.name)}${p.roundArtistBonus?'<span class="points-earned">★ artist</span>':''}</span><strong class="${p.roundPoints>0?'right':'wrong'}">${result}</strong></li>`}).join(""):"";list("leaderboard",s.players);
  }
