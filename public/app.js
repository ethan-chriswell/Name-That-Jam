(function(){
  ...
  $("next-round").onclick=()=>socket.emit("next-round");
  ...
  function list(target,players){$(target).innerHTML=players.map((p,i)=>`<li><span>${i+1}. ${escapeHtml(p.name)} ${p.answered?'<span class="answered">✓</span>':''}</span><strong>${p.score}</strong></li>`).join("")}
  ...
  function render(s){state=s;
    ...
    screen("game");$("round-label").textContent=`ROUND ${s.round} / ${s.totalRounds}`;$("decade-label").textContent=s.decade;$("host-stage").hidden=!s.isHost;$("countdown-panel").hidden=s.phase!=="countdown";$("question").hidden=s.phase!=="question";$("reveal-panel").hidden=s.phase!=="reveal";$("reveal").hidden=!s.isHost;$("next-round").hidden=!s.isHost;
    const tick=()=>{const remaining=s.deadline?Math.max(0,Math.ceil((s.deadline-Date.now())/1000)):0;$("timer").textContent=s.deadline?`${remaining}s`:"";if(s.phase==="countdown")$("countdown-number").textContent=remaining};tick();if(s.deadline)timerHandle=setInterval(tick,250);
    ...
    if(s.answer){$("answer-title").textContent=s.answer.title;$("answer-artist").textContent=s.answer.artist}list("leaderboard",s.players);
  }
